...
bash ./scripts/NEW_RL/${EXP_NO}/${DATASET_SC}_scripts/train_anomaly_transformer_fast_learner.sh
echo "Finished at `date`" >> $FL_RES
echo "---"
#
echo "Train DC Detector" >> $FL_RES
echo "Started at `date`" >> $FL_RES
bash ./scripts/NEW_RL/${EXP_NO}/${DATASET_SC}_scripts/train_dcdetector_fast_learner.sh
echo "Finished at `date`" >> $FL_RES
echo "---"
#
echo "Train Slow Learner" >> $FL_RES
echo "Started at `date`" >> $FL_RES
bash ./scripts/NEW_RL/${EXP_NO}/${DATASET_SC}_scripts/train_maelnet_slow_learner.sh
echo "Finished at `date`" >> $FL_RES
